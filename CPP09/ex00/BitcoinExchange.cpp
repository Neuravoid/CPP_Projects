...
static int seperateAndSave(std::map<std::string,float>& mapExample,std::string line, char type)
{
    ...
    if ((type == ',' && (fRight > 2147483647 || fRight < 0)) ||
        (type == '|' && (fRight > 1000 || fRight < 0))
    )
    {
        std::cout << fRight << " Number Too High or Too Low" << std::endl;
        return 1;
    }
...
void BitcoinExchange::getData(std::string fileName)
{
    ...
    {
        seperateAndSave(this->data,line,',');
    }
}
...
void BitcoinExchange::getInput(std::string fileName)
{
    ...
    while (std::getline(file,line))
    {
        result = seperateAndSave(this->input,line,'|');
        ...
        if (result == 0)
        {
            ...
            {           
                ...
                {                
                    ...
                    std::cout << std::fixed << std::setprecision(2) << input_ite->first << " => " << value << std::endl;
                }
                data_it++;
                }
                if (input_ite->first != data_it->first)
                {
                    ...
                    std::map<std::string,float>::iterator behindIt =  this->returnBound(input_ite->first);
                    value = behindIt->second * input_ite->second;
                    std::cout << std::fixed << std::setprecision(2) << this->input.begin()->first << " => " << value << std::endl;
                }
        }
    }
}
...
BitcoinExchange& BitcoinExchange::operator=(const BitcoinExchange& ref){
    this->data = ref.data;
    return *this;
    std::cout << "Copy Assignmant Operator Called" << std::endl;
}
